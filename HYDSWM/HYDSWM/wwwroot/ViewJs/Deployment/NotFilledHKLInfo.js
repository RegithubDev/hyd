...
function GetDataTableData(Type) {
    ...
    dt3 = $('#example').DataTable({
        ...
        ajax: {
            url: "/Deployment/GetAllNotFilledHKL_Paging/",
            type: 'POST',
            data: function (d) {
                d.Status = TSId;
                return {
        ...
        columns: [
            ...
            { data: "LastModifiedOn", sortable: true },
            {
                sortable: false,
                "render": function (data, type, row, meta) {
                    return "<a dhltid='" + row.DHLTId + "' href='javascript:void(0);' title='delete' onclick='RemoveHKL(this);'><i style='color:red;' class='icon-cancel-circle2'></i></a><br><a cid='" + row.DHLTId + "' href='javascript:void(0);' title='edit' onclick='CallFunc(this);'><i class='ti-pencil'></i></a>";
                }
            }
        ]
    });
...
function Formsubmit() {
    ...
    UpdateHKLLocationInfo();
    return false;
}
function RemoveHKL(obj) {

    var ddId = $(obj).attr('dhltid');

    $.ajax({
        type: "POST",
        url: '/Deployment/RemoveArvlHKLById',
        data: { DHLTId: ddId },
        success: function (data) {
            var myJson = JSON.parse(data);
            if (myJson.Result == 1 || myJson.Result == 2) {

                ShowCustomMessage('1', myJson.Msg, '/Deployment/AllNotFilledHKL');
...
function UpdateHKLLocationInfo() {
    ...
    if (isvalid == 1)
        $.ajax({
            type: "POST",
            url: '/Deployment/UpdateHKLLocation',
