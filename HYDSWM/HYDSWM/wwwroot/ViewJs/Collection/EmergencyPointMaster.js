    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var i = 0; i < Myjson.length; i++) {
                if (Myjson[i].GPCId != 5 && Myjson[i].GPCId != 6 && Myjson[i].GPCId != 7 )
...
function SetDataOncontrols(ddId) {
    $.ajax({
        type: "post",
        url: "/Collection/GetEmergencyPointMasterInfoById",
        data: { GeoPointId: ddId },
        success: function (data) {
            
            var myJSON = JSON.parse(data);
            $("#hfGPId").val(myJSON.GeoPointId);
            $("#ddlPointType").val(myJSON.PointType);
            if ($("#ddlPointType").val() == 2) {
                ...
                $("#dvtxtPointName").hide();
                $("#dvPointName").show();
                //$("#ddlPointName").attr(myJSON.GeoPointName);//ChangeS
                $("#ddlPointName option:selected").text(myJSON.GeoPointName)
            }
...
function SaveAndUpdateGeoPointInfo() {
    ...
    if ($("#ddlPointType").val() == 1) {
        ...
        pointnameS = $("#txtPointName").val();
    }
    var input = {
        GeoPointId: $("#hfGPId").val(),
        GeoPointName: pointnameS,
        ...
        WardId: $("#ddlWard").val(),
        PointType: $("#ddlPointType").val(),
    };


    if (input.GeoPointName == '' || input.CategoryId == '' || input.Remarks == '' || input.Radius == '' || input.Lat == '' || input.Lng == '' )
        isvalid = 0;
    ...
    if (isvalid == 1)
        $.ajax({
            type: "POST",
            url: '/Collection/AddEmergencyLocationSurvey',
            // dataType: "json",
            data: formData,
            contentType: false,
            processData: false,
            success: function (response) {
                ...
                if (myJSON.Result == 1 || myJSON.Result == 2) {
                    ...
                    ShowCustomMessage('1', myJSON.Msg, '/Collection/AllEmergencyPoint');
...
function GetDataTableData(Type) {
    ...
    dt = $('#example').DataTable({
        ...
        ajax: {
            url: "/Collection/GetAllEmergencyPoint_Paging/",
            type: 'POST',
            data: function (d) {
                d.ZoneId = ZoneId;
                d.CircleId = CircleId;
        ...
        columns: [
            ...
            //},
            {
                ...
                "render": function (data, type, row, meta) {
                    ...
                    return "<a cid='" + row.Id + "'   href='javascript:void(0);' title='edit' onclick='CallFunc(this);'><i class='ti-pencil'></i></a>";
            ...
            { data: "WardNo", sortable: true },
            {
                ...
                "render": function (data, type, row, meta) {
                    ...
                    return "<a data-GeoPointName='" + row.GeoPointName +
                        ...
                        "'data-Location='" + row.GeoPointCategory +
                        "'data-Lat='" + row.Lat +
                        "'data-Lng='" + row.Lng +
...
function GetGeopointDataForMap(Type) {
    ...
    $.ajax({
        ...
        url: '/Collection/AllEmergencyPointsMapInfo',
        dataType: "json",
...
function GetPointsName() {
    ...
    $.ajax({
        type: "post",
        url: "/Collection/GetAllDEmergencyPointsName",
...
$(document).on("change", "#ddlPointName", function () {


    GrepData();
})
function GrepData() {
    var dataS = $.grep(MyjsonMain, function (element, index) {
        return element.GeoPointId == $("#ddlPointName").val();
        // console.log(data.items);
    });
    
    var data1 = dataS;
    $("#txtPointId").val(data1[0].GeoPointId);
    $("#txtLat").val(data1[0].Lat);
