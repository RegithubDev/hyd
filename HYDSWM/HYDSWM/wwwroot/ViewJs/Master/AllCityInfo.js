$(document).ready(function () {
    //AllMCityLst('ddlSLACategoryType',1,'Select');
...
function bindtable(data) {
    ...
    $.each(data, function (i, item) {
        ...
        var edit;
        if (CId == '') {
            edit = "<td><a cid='" + item.CId + "' href='javascript: void (0); ' title='edit' onclick='CallFunc(this,1); '><i class='ti-pencil'></i></a></td>";
        }
        else {
            edit = "<td></td>";
...
function CallFunc(obj,x) {
    ...
    $('#user_content').load("/Master/AddCity?add_upd=" + x);
...
function SetDataOnControls(ddId) {
    $.ajax({
        type: "post",
        url: "/Master/GetCityInfoById",
        data: { CId: ddId },
        success: function (data) {

            var myJSON = JSON.parse(data);
            $("#hfCId").val(myJSON.CId);
            $("#txtCity").val(myJSON.CityNo);
            $("#txtCityCode").val(myJSON.Citycode);
...
function SaveAndUpdateCity() {
    ...
    if (FormData.CityNo == '' || FormData.Citycode == '')
        isvalid = 0;

    if (isvalid == 1)
        $.ajax({
            type: "POST",
            url: '/Master/SaveAndUpdateCity',
            data: { jobj: JSON.stringify(FormData) },
            success: function (data) {
                var myJson = JSON.parse(data);
                if (myJson.Result == 1 || myJson.Result == 2) {

                    ShowCustomMessage('1', myJson.Msg, '/Master/AllCity');
