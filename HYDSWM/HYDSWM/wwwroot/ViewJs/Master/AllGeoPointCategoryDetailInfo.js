...
function SetValues() {
    ...
    $.ajax({
        ...
        url: '/Master/GetAllGeoPointCategoryDetailInfo',
        dataType: "json",
        data: { IsAll: "YES" },
...
function bindtable(data) {
    ...
    $.each(data, function (i, item) {
        ...
        var edit;
        if (CId == '') {
            edit = "<td><a cid='" + item.GPCId + "' href='javascript: void (0); ' title='edit' onclick='CallFunc(this); '><i class='ti-pencil'></i></a></td>";
        }
        else {
            edit = "<td></td>";
...
function CallFunc(obj) {
    ...
    $('#user_content').load("/Master/AddGeoPointCategoryDetail");
...
function SetDataOnControls(ddId) {
    $.ajax({
        type: "post",
        url: "/Master/GetGeoPointCategoryDetailInfoById",
        data: { GPCId: ddId },
        success: function (data) {

            var myJSON = JSON.parse(data);
            $("#hfGPCId").val(myJSON.GPCId);
            $("#txtCategory").val(myJSON.GeoPointCategory);
...
function Formsubmit() {
    SaveAndUpdateGeoPointCategory();
    return false;
}
function SaveAndUpdateGeoPointCategory() {
    var isvalid = 1;
    var FormData = {
        GPCId: $("#hfGPCId").val(),
        GeoPointCategory: $("#txtCategory").val(),
        IsActive: $('#ckbIsActive').is(':checked')
    };
    if (FormData.GeoPointCategory == '')
        isvalid = 0;

    if (isvalid == 1)
        $.ajax({
            type: "POST",
            url: '/Master/SaveAndUpdateGeoPointCategoryDetail',
            data: { jobj: JSON.stringify(FormData) },
            success: function (data) {
                var myJson = JSON.parse(data);
                if (myJson.Result == 1 || myJson.Result == 2) {

                    ShowCustomMessage('1', myJson.Msg, '/Master/AllGeoPointCategoryDetail');
